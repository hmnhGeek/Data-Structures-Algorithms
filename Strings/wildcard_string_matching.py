        ...
        for i in dp:
            ...
            for k in range(i, 0, -1):
                ...
                if s1[k - 1] != "*":
                    got_all_stars = False
                    break
            dp[i][0] = got_all_stars

        for i in range(1, n1 + 1):
            for j in range(1, n2 + 1):
                # if characters from both strings match or the character in the pattern string is a "?", then there
                # is a match, call the recursion for next characters in both the strings.
                if s1[i - 1] == s2[j - 1] or s1[i - 1] == "?":
                    dp[i][j] = dp[i - 1][j - 1]
                elif s1[i - 1] == "*":
                    # if the current character from the pattern string is a "*", then the * can match with either 0
                    # characters from the other string, or a number of continuous characters from the other string.

                    # if we say that * does not match with any character in the other string, then simply ignore the
                    # * and move to the next character in the pattern, but stay on the same character in the other
                    # string.
                    star_matches_no_one = dp[i - 1][j]

                    # if we say that * can match with the current character of the other string, then we match the
                    # other string's character by moving to the next character in the other string but staying at the
                    # * in the pattern string, because we need to check if * can match with the next character of the
                    # other string as well and so on.
                    star_matches_with_current_char_in_other_string = dp[i][j - 1]

                    # take a logical OR of both the cases and return. Basically, return a true if either of the ways
                    # the string match.
                    dp[i][j] = star_matches_no_one or star_matches_with_current_char_in_other_string
                else:
                    dp[i][j] = False
        ...
        for i in range(1, n1 + 1):
            ...
            for j in range(1, n2 + 1):
                ...
                if s1[i - 1] == s2[j - 1] or s1[i - 1] == "?":
                    ...
                    # * and move to the next character in the pattern, but stay on the same character in the other
                    # string.
                    star_matches_no_one = prev[j]

                    # if we say that * can match with the current character of the other string, then we match the
                    # other string's character by moving to the next character in the other string but staying at the
                    # * in the pattern string, because we need to check if * can match with the next character of the
                    # other string as well and so on.
                    star_matches_with_current_char_in_other_string = curr[j - 1]

                    # take a logical OR of both the cases and return. Basically, return a true if either of the ways
                    # the string match.
                    curr[j] = star_matches_no_one or star_matches_with_current_char_in_other_string
                else:
                    curr[j] = False
            prev = curr
