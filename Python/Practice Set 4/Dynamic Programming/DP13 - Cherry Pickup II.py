...
# Solution - https://www.youtube.com/watch?v=QGfn7JeXK54&list=PLgUwDviBIf0qUlt5H_kiKYaNSqJ81PMMY&index=14
                        ...
                        for b in range(m):
                            ...
                            for da in range(-1, 2):
                                for db in range(-1, 2):
                                    if 0 <= a + da < m and 0 <= b + db < m:
                                        if a == b:
                                            chocolates = max(chocolates, mtx[i][a] + prev[a + da][b + db])
                                        else:
                                            chocolates = max(chocolates, mtx[i][a] + mtx[i][b] + prev[a + da][b + db])
                            curr[a][b] = chocolates
                    prev = curr

                max_chocolates = max(max_chocolates, prev[last_alice_index][last_bob_index])
        return max_chocolates
