...
class BinarySearchTree:
    ...
    def recalc_augmentation(self, parent):
        ...
        while parent is not None:
            left_sz = parent.left.size if parent.left is not None else 0
            left_ht = parent.left.ht if parent.left is not None else 0
            right_sz = parent.right.size if parent.right is not None else 0
            right_ht = parent.right.ht if parent.right is not None else 0
    ...
    def get_predecessor(self, node):
        ...
        if node.left is not None:
            return self.get_rightmost_node(node.left)
        parent = node.parent
        if parent is None:
            return
