# Problem link - https://www.geeksforgeeks.org/eventual-safe-states/
...
def safe_nodes_by_dfs_topo_sort():
...
def safe_nodes_without_topo_sort():
    def dfs(graph, node, visited, path_visited, check):
        # mark the node as visited and path visited.
        visited[node] = True
        path_visited[node] = True

        # ensure that for now, the node is unsafe
        check[node] = False

        for adj_node in graph[node]:
            # if the node is not visited, run a dfs on it.
            if not visited[adj_node]:
                if dfs(graph, adj_node, visited, path_visited, check):
                    # at any point, if a cycle is detected, return True.
                    # this is indeed an unsafe node, ensure check is false for this adjacent node.
                    check[adj_node] = False
                    return True

            # if the node is visited and path visited also, there's a cycle, return True and uncheck as safe node.
            if path_visited[adj_node]:
                check[adj_node] = False
                return True

        # unmark the node from path visited, because it's a safe node and we don't want it to give false negative
        # (that there's a cycle) where there is another path leading to the same safe node. Return false, no cycle
        # is there.
        check[node] = True
        path_visited[node] = False
        return False
    ...
    def get_safe_nodes(graph):
        ...
        # check node will be used to track safe nodes. All the true values will denote safe nodes.
        check = {i: False for i in graph}

        # to run on all the connected components.
        for node in graph:
            if not visited[node]:
                dfs(graph, node, visited, path_visited, check)

        print([k for k, v in check.items() if v])
...
safe_nodes_by_dfs_topo_sort()
print()
safe_nodes_without_topo_sort()