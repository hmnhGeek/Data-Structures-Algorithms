...
class Solution:
    ...
    def get_fractional_knapsack(values, weights, capacity):
        ...
        # extra space.
        arr = [(values[i], weights[i], i + 1) for i in range(n)]
        ...
        # store collected items position in this array
        collected_items_order = []

        # now loop on each item in O(n) time.
        for item in arr:
            value, weight, item_position = item
            ...
            # add the collected item into the array
            collected_items_order.append(item_position)
        ...
        return collected_value, collected_items_order
