...
public class BinarySearchTree<T extends Comparable<T>> {
    ...
    private void recalculateAugmentation(Node<T> parent) {
        ...
        while (parent != null) {
            ...
            Integer leftHt = parent.getLeft() != null ? parent.getLeft().getHeight() : 0;
            Integer rightHt = parent.getRight() != null ? parent.getRight().getHeight() : 0;
            parent.setSize(leftSize + rightSize + 1);
            parent.setHeight(1 + Math.max(leftHt, rightHt));
            parent.setDiameter(1 + leftHt + rightHt);
    ...
    private void delete(Node<T> node) {
        ...
        if (node.getRight() != null) {
            ...
            T successorData = successor.getData(), nodeData = node.getData();
            successor.setData(nodeData);
            node.setData(successorData);
            delete(successor);
            return;
        }
        Node<T> predecessor = getPredecessor(node);
        T predecessorData = predecessor.getData(), nodeData = node.getData();
        predecessor.setData(nodeData);
