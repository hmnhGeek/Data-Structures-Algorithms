...
public class BinarySearchTree<T extends Comparable<T>> {
    ...
    private Node<T> getNode(Node<T> start, T x) {
        ...
        if (x == start.getData()) return start;
        if (x.compareTo(start.getData()) > 0) return getNode(start.getRight(), x);
    ...
    private void show(Node<T> start) {
        if (start != null) {
            ...
            if (start == this.root) {
                stringBuilder.append(String.format("Data = %s (root), ", start.getData()));
            } else {
                stringBuilder.append(String.format("Data = %s, ", start.getData()));
            }
