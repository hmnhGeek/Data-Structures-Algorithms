...
def traverse(arr1, arr2, row, col):
    ...
    # again, one option is to continue on the same array; we will call it left recursion; just move to previous index
    # on the same array.
    ...
    if col == 0:
        ...
        curr_val = arr1[row]
        # get the index of the same cell value in the other array, if the value exists, otherwise we will get a None.
    ...
    # if the same cell value is found in the other array as well, then we must have an intersection. The right recursion
    # would be row - 1 of the other array, i.e., switch_idx - 1 of the other array, because the common value is already
    # taken from this array (on which we are right now).
